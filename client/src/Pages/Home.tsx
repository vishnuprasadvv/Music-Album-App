...
const Home: React.FC = () => {
  ...
  //Filter collections based on search and selected filters
  const filteredCollections = collections.filter((c) => {
    const matchesSearch = c.name
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesFilter =
      selectedItems.length === 0 || selectedItems.includes(c.type);
      ...
      <div className="content border-2 border-[#E6ECF0] bg-white m-3 rounded-lg p-3">
        ...
        <div className="pb-3 flex gap-3 h-[45px]">
          ...
          <div className="relative h-full" ref={dropDownRef}>
            ...
            >
              Type {selectedItems.length > 0 && `(${selectedItems.length})`}
            ...
            <tbody>
              {filteredCollections.map((collection) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    {Math.floor(collection.durationInSeconds / 60)} mins
                  </td>
                  <td className="py-3 px-4">
